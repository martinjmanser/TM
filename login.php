...
if (isset($_POST['loggedin'])) {
	...
	$sql = "SELECT COUNT(*) as total, id, password FROM users WHERE email = '$userEmail'";
	...
	if ($result['total'] == 0) { ?>
		...
		<br />
		<? echo "Username does not exist, please try again <br /><br />"; ?>
		</div> 	
...
?>
		<div id="container">
			...
			<?php } else if (isset($_GET['justLoggedOut'])) { ?>
				...
				<br /><br />
			<?php } ?>
			<p>Enter your username and password to login.</p><br /><br />
			<form method="post">
				<input type="hidden" name="loggedin" value="1" />
				<div class="input-group">
					<input type="text" class="form-control" name="user" placeholder="Username" style="height:50px;border-bottom-left-radius: 0" />
					<input type="password" class="form-control" name="password" placeholder="Password" style="height:50px;border-bottom-left-radius: 4px;" />
					<span class="input-group-btn">
						<input type="submit" value="Login" style="height: 100px;" class="btn btn-primary" />
					</span>
				</div>
			</form>
		</div>
